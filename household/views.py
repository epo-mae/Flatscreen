...
from .appearance import PRESETS, appearance_attrs, build_appearance_css, build_display_css, display_attrs, overrides_diff, preset_config, resolve_config, theme_color
...
def settings_page(request):
    ...
    appearance_form = AppearanceForm(config=house.appearance_resolved)
    if request.method == 'POST':
        ...
        with transaction.atomic():
            if action == 'household':
                ...
                if preset_key in PRESETS:
                    changed_preset = preset_key != house.base_preset
                    house.base_preset = preset_key
                    house.appearance_overrides = overrides_diff(preset_key, house.appearance_overrides)
                    house.save(update_fields=['base_preset', 'appearance_overrides'])
                    if changed_preset:
                ...
                if appearance_form.is_valid():
                    house.appearance_overrides = overrides_diff(house.base_preset, appearance_form.cleaned_overrides())
                    house.save(update_fields=['appearance_overrides'])
                    ActivityEntry.objects.create(actor=request.user, message='Fine-tuned the household design system')
                    bump_revision()
    ...
    if preview_key in PRESETS:
        preview_config = preset_config(preview_key)
        appearance_context['appearance_preview'] = {
            ...
            'display_stylesheet': f'/appearance-display.css?preset={preview_key}',
        }
        appearance_context['appearance_form'] = AppearanceForm(config=preview_config)
        appearance_context['appearance_attrs'] = appearance_attrs(preview_config)
        appearance_context['appearance_display_attrs'] = display_attrs(preview_config)
        appearance_context['theme_color'] = theme_color(preview_config)
...
def appearance_css(request):
    ...
    if preset_key in PRESETS:
        ...
        config = house.appearance_resolved if house else resolve_config('classic')
    css = build_appearance_css(config)
...
@require_GET
def appearance_display_css(request):
    """Tokens for the household display wall, cached by the display's worker."""
    preset_key = request.GET.get('preset')
    if preset_key in PRESETS:
        config = preset_config(preset_key)
    else:
